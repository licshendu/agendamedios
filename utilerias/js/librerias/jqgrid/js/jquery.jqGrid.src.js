        ...
        return this.each(function () {
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                    ...
                    updatepager = function (rn, dnd) {
                        ...
                        if (pgboxes) {
                            ...
                            if (ts.p.pginput === true) {
                                $('.ui-pg-paginador', pgboxes).val(ts.p.page);
                                $('#page_active').html(ts.p.page);
                    ...
                    setPager = function (pgid, tp) {
                        ...
                        if (ts.p.pginput === true) {
                            pginp = "<td dir='" + dir + "'>" + $.jgrid.format(ts.p.pgtext || "", "<label class='ui-pg-paginador' type='text' size='2' maxlength='7' id='page_active' value='0' role='textbox'> </label>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>") + "</td>";
                        }
                        if (ts.p.pgbuttons === true) {
                        ...
                        if (ts.p.pginput === true) {
                            $('label.ui-pg-paginador', "#" + pgcnt).keypress(function (e) {
            ...
            return this.each(function () {
                    ...
                    if (p.stringResult === true || $t.p.datatype === "local") {
                        ...
                        if (p.urlAct == true) {
                            ...
                            for (name in sdata) {
                                ...
                                copy = sdata[ name ];
                                cadena = cadena + name + "=" + copy
                                if (i != length) {
                                    cadena = cadena + ",";
                                }
                                i++;
                            }
                            var term = {'term': cadena};
                            $.extend($t.p.postData, term);
                        } else if (p.filtro == true) {
                ...
                $.each($t.p.colModel, function (ci) {
                    ...
                    if (this.stype != "select") {
                        cleanInput = "<td class='ui-search-clear'></td>";
                    }
                    var cm = this, soptions, surl, self, select = "", sot = "=", so, i,
                            th = $("<th role='columnheader' class='ui-state-default ui-th-column ui-th-" + $t.p.direction + "'></th>"),
                            thd = $("<div style='position:relative;height:100%;padding-right:0.3em;padding-left:0.3em;'></div>"),
                            stbl = $("<table class='ui-search-table' cellspacing='0'><tr><td class='ui-search-oper'></td><td class='ui-search-input'>" + cleanInput + "</td></tr></table>");
                    if (this.hidden === true) {
                        $(th).css("display", "none");
                    }
                        ...
                        switch (this.stype)
                        {
                            ...
                            case "text"://aquiui
                                ...
                                if (this.onkeypress !== undefined) {
                                    keypress = "onkeypress=\"" + this.onkeypress + "\"";
                                }
                                ...
                                if (this.isNumerico) {
                                    ...
                                    if (this.isFlotante) {
                                        numerico = numerico + "step=\"any\" ";
                                    }
                                    if (this.isPositivo)
                                    {
                                        numerico = numerico + "min=\"0\" ";
                                    }

                                    if (this.isNegativo)
                                    {
                                        numerico = numerico + "max=\"0\" ";
                                    }
                                }
                                $("td:eq(1)", stbl).append("<input " + tipo + " style='width:100%;padding:0px;' rel = 'campo_busqueda' name='" + (cm.index || cm.name) + "' id='gs_" + cm.name + "' value='" + df + "' style=\"text-transform:uppercase;\" onkeyup=\"javascript:this.value=this.value.toUpperCase();\" " + keypress + " " + numerico + " />");
                                $(thd).append(stbl);
                                ...
                                if (p.autosearch === true) {
                                    if (p.searchOnEnter) {
                                        $("input", thd).keypress(function (e) {
                                            var key = e.charCode || e.keyCode || 0;
                                            if (key === 13) {
                                                if ($t.p.urlOriginal != null) {
                                                    $t.p.url = $t.p.urlOriginal;
                                                }
                                                triggerToolbar();
                                                return false;
                                            }
